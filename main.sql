...
CREATE TABLE Employee (
  ...
  BossNumber SMALLINT UNSIGNED REFERENCES Employee, -- this was marked NOT NULL, but then we couldn't model a boss as they'd have no BossNumber...
...
-- (DepartmentName), so it performs the correct equijoin implicitly.
...
-- this version gives the same results as above; the implicit join/on cols with same name
SELECT DISTINCT ItemName FROM (Sale NATURAL JOIN Department) WHERE Department.DepartmentFloor = 2;

-- JOIN without an ON clause produces a Cartesian product.
-- Therefore the query returns different results because the tables
-- are not actually joined on DepartmentName.
SELECT DISTINCT ItemName FROM (Sale JOIN Department) WHERE Department.DepartmentFloor = 2;
...
SELECT DepartmentName, AVG(EmployeeSalary) AS 'Average Salary' FROM Employee GROUP BY DepartmentName ORDER BY 'Average Salary' DESC;
...
Supplier.SupplierNumber GROUP BY Supplier.SupplierNumber, Supplier.SupplierName HAVING COUNT(DISTINCT Delivery.ItemName) >= 10;
...
-- NATURAL JOIN automatically joins tables using columns with the same
-- names (DepartmentName and ItemName here), so this query is equivalent
-- to the original equijoin query and produces the same result.
...
-- Empty result because no department has sold the item 'Stetson',
-- even though some departments have total salaries greater than £25000.
-- also changed ItemName to reflect reality; Stetson, not Stetsons
